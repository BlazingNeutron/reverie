...
export async function subscribe(docId: string, callback: Function) {
    const channel = supabase.channel(`yjs-${docId}`)
...
export async function broadcastAwareness(docId: string, base64Awareness: string) {
...
export async function subscribeAwareness(docId: string, callback: Function) {
    supabase.channel(`yjs-${docId}`)
        .on('broadcast', { event: 'awareness' }, (payload: { payload: { awareness: string } }) => {
