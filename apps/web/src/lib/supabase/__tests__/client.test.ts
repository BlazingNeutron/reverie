...
describe("supabase client", () => {
    ...
    it("supabase called with baseUrl and key", async () => {
        ...
        expect(mockCreateClient).toHaveBeenCalledWith([ "http://test.com", "averyspecialtestkey2" ]);
    })
    ...
    it("supabase session exists", async () => {
        await setImportClient("key", { user: { id: 'test-user' }, access_token: "test_access_token" });
        expect(mockCreateClient).toHaveBeenCalledTimes(2);
        expect(mockCreateClient).toHaveBeenCalledWith([ "http://test.com", "test_access_token" ]);
    })

    async function setImportClient(key : string, session? : any) {
        supabaseClientMock.auth.getSession = vi.fn().mockReturnValue({ data: { "session": session } })
